...
public class SetPlayersCommand extends FastBridgerCommand {

    private Main main;

    public SetPlayersCommand(Main main) {
        this.main = main;
    }
    ...
    public void executeAs(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        try {
            int index = Integer.parseInt(maxPlayers);
        } catch (Exception exception) {

        }

        if (main.getFiles().getArenasConfig().contains(arena)) {
            main.getFiles().getArenasConfig().set(arena + ".max-players", maxPlayers);
            ...
            player.sendMessage("You have successfully set the maximum amount of players on the arena called " + arena + " to " + maxPlayers);
        } else {
